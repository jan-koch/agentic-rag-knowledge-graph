...
ALLOWED_N8N_IPS = (
    os.getenv("ALLOWED_N8N_IPS", "").split(",") if os.getenv("ALLOWED_N8N_IPS") else []
)
...
def verify_n8n_webhook_signature(payload: bytes, signature: str, secret: str) -> bool:
    """
    ...
    if not secret:
        logger.warning(
            "N8N_WEBHOOK_SECRET not configured - signature verification disabled"
        )
        return True
...
def validate_n8n_request(request: Request) -> Dict[str, Any]:
    ...
    if any(agent in user_agent for agent in suspicious_agents):
        ...
        if "n8n" not in user_agent and not any(
            ...
            raise SecurityError(
                status_code=403, detail="Access denied - invalid user agent"
            )
...
async def verify_api_key(
    ...
    if not hmac.compare_digest(credentials.credentials, API_KEY):
        logger.warning(
            f"Invalid API key attempted from {credentials.credentials[:8]}..."
        )
        raise HTTPException(
            status_code=401,
...
async def verify_n8n_api_key(
    ...
    if not credentials:
        raise SecurityError(status_code=401, detail="Missing API key")
...
def require_n8n_auth(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            raise SecurityError(status_code=500, detail="Security validation failed")
